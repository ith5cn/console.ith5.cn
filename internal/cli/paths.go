...
func (p Paths) EnsureDirs() error {
	for _, d := range []string{p.Home, p.Store, p.ClaudeHome, p.Skills, p.Agents, p.Staging, p.Trash} {
...
//
// 目录形态是 skills/<name>/，文件形态是 agents/<name>.md。
func (p Paths) Target(shape core.Shape, bundleName string) string {
	if shape == core.ShapeFile {
		return filepath.Join(p.Agents, bundleName+".md")
	}
	return filepath.Join(p.Skills, bundleName)
}

// RelTarget 返回用于回执 detail 的相对化路径。
// 绝对路径含 home，与 L0 白名单同一约束，不得上报（技术方案 §5.2）。
func (p Paths) RelTarget(shape core.Shape, bundleName string) string {
	if shape == core.ShapeFile {
		return "agents/" + bundleName + ".md"
	}
	return "skills/" + bundleName
}
