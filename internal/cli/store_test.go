...
func TestStore_WriteAndHas(t *testing.T) {
	...
	if s.Has(core.ShapeDir, "corp-a", sum) {
		t.Fatal("写入前不应存在")
	}
	...
	got, err := os.ReadFile(filepath.Join(s.Dir("corp-a", sum), "references", "api.md"))
...
func TestStore_SameContentSharesDirectory(t *testing.T) {
	...
	entries, _ := os.ReadDir(filepath.Join(s.Root(), "corp-a"))
	if len(entries) != 1 {
...
func TestStore_Prune(t *testing.T) {
	...
	// 保留当前在用的那个，总量上限 2
	if err := s.Prune("corp-a", []string{sums[3]}, 2); err != nil {
		t.Fatal(err)
	}
	if !s.Has(core.ShapeDir, "corp-a", sums[3]) {
		t.Fatal("在用的内容必须保留")
	}
	entries, _ := os.ReadDir(filepath.Join(s.Root(), "corp-a"))
	if len(entries) > 2 {
...
func TestLock_重建扫两种形态(t *testing.T) {
	...
	for _, c := range []struct {
		...
		shape core.Shape
		sum   string
	}{{"sk", core.ShapeDir, skillSum}, {"ag", core.ShapeFile, agentSum}} {
		err := strategies.For(c.shape).Materialize(
			st.Dir(c.name, c.sum), p.Target(c.shape, c.name),
			link.MarkerData{BundleID: "b-" + c.name, BundleName: c.name, Version: 1, Checksum: c.sum})
		if err != nil {
	...
	if got := l.Bundles["sk"].Shape; got != core.ShapeDir {
		t.Errorf("sk 的形态应为 dir，实际 %q", got)
	}
	if got := l.Bundles["ag"].Shape; got != core.ShapeFile {
		t.Errorf("ag 的形态应为 file，实际 %q", got)
	}
	if l.Bundles["ag"].ShortSum != core.ShortSum(agentSum) {
		t.Errorf("agent 应由内容比对反推出摘要，实际 %q", l.Bundles["ag"].ShortSum)
	}
	if _, ok := l.Bundles["his-own"]; ok {
		t.Error("员工自建的内容被认领了")
	}
}

func TestOwnerships_按形态探测正确路径(t *testing.T) {
	root := t.TempDir()
	p := Paths{
		Home: filepath.Join(root, "ith5"), Store: filepath.Join(root, "ith5", "store"),
		ClaudeHome: filepath.Join(root, "claude"),
	...
	// 形态搞错的话会去 skills/x 探测，得出 absent 然后重装一遍。
	os.WriteFile(filepath.Join(p.Agents, "x.md"), []byte("用户自己的"), 0o644)

	own, err := Ownerships(p, NewStrategies("copy", p), []string{"x"},
		map[string]core.Shape{"x": core.ShapeFile})
	if err != nil {
		t.Fatal(err)
	}
	if own["x"] != core.OwnForeign {
		t.Fatalf("应探测到 agents/x.md 并判为用户自有，实际 %v", own["x"])
	}
}
