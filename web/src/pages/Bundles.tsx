...
//
// entry 是**必须**用的文件名：种错了要到发布时才报错，而那时正文已经
// 写进了一个错名文件里。
const KINDS: { value: string; label: string; entry: string; hint: string }[] = [
  { value: 'skill', label: 'skill — 可自动调用', entry: 'SKILL.md', hint: '落到 ~/.claude/skills/<名称>/，可带支持文件。' },
  { value: 'command', label: 'command — 用户主动触发', entry: 'SKILL.md', hint: '同 skill，但正文须含 disable-model-invocation: true。' },
  { value: 'agent', label: 'agent — 可派发的 subagent', entry: 'AGENT.md', hint: '落到 ~/.claude/agents/<名称>.md，只能一个文件；frontmatter 的 name 必须等于名称。' },
  { value: 'hook', label: 'hook — 钩子脚本', entry: 'HOOK.js', hint: '落到 ~/.claude/hooks/<名称>.js。脚本落盘不等于生效，还需一个 setting 类型把它登记进 settings.json。' },
  { value: 'workflow', label: 'workflow — Workflow 脚本', entry: 'WORKFLOW.js', hint: '落到 ~/.claude/workflows/<名称>.js。' },
  { value: 'standard', label: 'standard — 团队规范文档', entry: 'STANDARD.md', hint: '落到 ~/.claude/standards/<名称>.md。不会被自动加载，靠 CLAUDE.md 或 skill 正文引用。' },
  { value: 'setting', label: 'setting — settings.json 片段', entry: 'SETTING.json', hint: '按顶层键合并进 ~/.claude/settings.json。用户改过的键不会被覆盖。' },
  { value: 'mcp', label: 'mcp — MCP server 定义', entry: 'MCP.json', hint: '按 server 名合并进 ~/.claude.json 的 mcpServers。' },
]

const kindMeta = (k: string) => KINDS.find((x) => x.value === k) ?? KINDS[0]

function seedFor(kind: string, name: string): string {
  switch (kind) {
    case 'command': return '---\ndescription: 描述\ndisable-model-invocation: true\n---\n\n在这里写下命令步骤。'
    case 'agent': return `---\nname: ${name || 'agent-name'}\ndescription: 描述\n---\n\n在这里写下 subagent 的职责。`
    case 'setting': return '{\n  "env": {\n    "EXAMPLE": "值"\n  }\n}'
    case 'mcp': return `{\n  "mcpServers": {\n    "${name || 'server-name'}": {\n      "command": "npx",\n      "args": ["-y", "某个-mcp-server"]\n    }\n  }\n}`
    case 'hook': return '#!/usr/bin/env node\n// 钩子从 stdin 读事件 JSON，向 stdout 写结果。'
    case 'workflow': return `export const meta = {\n  name: '${name || 'workflow-name'}',\n  description: '描述',\n}`
    case 'standard': return `# ${name || '规范标题'}\n\n在这里写下团队约定。`
...
function Create({ open, onOpenChange, onDone }: { open: boolean; onOpenChange: (open: boolean) => void; onDone: (id: string) => void }) {
  ...
  const placeholder = seedFor(kind, name)
